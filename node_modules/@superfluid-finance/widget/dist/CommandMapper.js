...
import { useBlockNumber, useContractRead, useContractReads, usePublicClient, useQuery, } from "wagmi";
import { autoWrapManagerABI, autoWrapManagerAddress, autoWrapStrategyAddress, cfAv1ForwarderABI, cfAv1ForwarderAddress, erc20ABI, mapTimePeriodToSeconds, nativeAssetSuperTokenABI, superTokenABI, } from "./core/index.js";
import { MaxUint256 } from "./utils.js";
import { useWidget } from "./WidgetContext.js";
export function CommandMapper({ command: cmd, ...props }) {
    switch (cmd.type) {
        case "Enable Auto-Wrap":
            return _jsx(EnableAutoWrapCommandMapper, { command: cmd, ...props });
        case "Wrap into Super Tokens":
            return _jsx(WrapIntoSuperTokensCommandMapper, { command: cmd, ...props });
        case "Subscribe":
            return _jsx(SubscribeCommandMapper, { command: cmd, ...props });
    }
}
export function EnableAutoWrapCommandMapper({ command: cmd, onMapped, }) {
    ...
    const contractWrites = useMemo(() => {
        ...
        if (typeof wrapSchedule === "object") {
            ...
            if (typeof allowance === "bigint" && allowance < upperLimit) {
                contractWrites_.push(createContractWrite({
                    commandId: cmd.id,
                    displayTitle: `Approve ${getUnderlyingToken(cmd.chainId, cmd.underlyingTokenAddress).symbol} Allowance`,
                    chainId: cmd.chainId,
                    abi: erc20ABI,
    ...
    useEffect(() => isSuccess ? onMapped?.({ commandId: cmd.id, contractWrites }) : void 0, [cmd.id, contractWrites, isSuccess]);
    return null;
}
export function WrapIntoSuperTokensCommandMapper({ command: cmd, onMapped, }) {
    const { getSuperToken, getUnderlyingToken } = useWidget();
    const isNativeAssetUnderlyingToken = cmd.underlyingToken.isNativeAsset;
    const { data: allowance_, isSuccess: isSuccess_ } = useContractRead(!isNativeAssetUnderlyingToken // ERC-20 allowance doesn't apply to native asset tokens
        ? {
            chainId: cmd.chainId,
    ...
    const contractWrites = useMemo(() => {
        ...
        if (isNativeAssetUnderlyingToken) {
            contractWrites_.push(createContractWrite({
                commandId: cmd.id,
                displayTitle: `Wrap to ${getSuperToken(cmd.chainId, cmd.superTokenAddress).symbol}`,
                chainId: cmd.chainId,
            ...
            if (allowance_ !== undefined) {
                ...
                if (allowance < cmd.amountWeiFromUnderlyingTokenDecimals) {
                    contractWrites_.push(createContractWrite({
                        commandId: cmd.id,
                        displayTitle: `Approve ${getUnderlyingToken(cmd.chainId, cmd.underlyingToken.address)
                            .symbol} Allowance`,
                        chainId: cmd.chainId,
                        abi: erc20ABI,
                        functionName: "approve",
                ...
                contractWrites_.push(createContractWrite({
                    commandId: cmd.id,
                    displayTitle: `Wrap ${getUnderlyingToken(cmd.chainId, cmd.underlyingToken.address)
                        .symbol} into ${getSuperToken(cmd.chainId, cmd.superTokenAddress).symbol}`,
                    chainId: cmd.chainId,
                    abi: superTokenABI,
                    address: cmd.superTokenAddress,
...
export const calculateNewFlowRate = ({ existingFlowRateWei: existingFlowRate, paymentFlowRate, modifyBehaviour, }) => {
    const paymentFlowRateWei = paymentFlowRate.amountWei /
...
export function SubscribeCommandMapper({ command: cmd, onMapped, }) {
    const { isSuccess: isSuccessForGetFlowRate, isLoading: isLoadingForGetFlowRate, data: existingFlowRate_, } = useContractRead({
        chainId: cmd.chainId,
    ...
    const { isSuccess: isSuccessForBlockNumber, isIdle: isIdleForBlockNumber, data: blockNumber, } = useBlockNumber({
        chainId: cmd.chainId,
    ...
    const { isSuccess: isSuccessForTransferEvents, isLoading: isLoadingForTransferEvents, isIdle: isIdleForTransferEvents, data: transferEvents, } = useQuery([cmd.id, blockNumber], async () => {
        ...
        const logs = await publicClient.getLogs({
            ...
            strict: true,
            toBlock: blockNumber,
            fromBlock: blockNumber - 5000n, // 10k is Alchemy limit, some places have 50k. Ankr has 5k. Move into environment variable?
        });
        return logs;
    }, {
    ...
    const skipTransfer = useMemo(() => {
        if (checkExistingUpfrontTransfer && isSuccessForTransferEvents) {
            ...
            return transferEvents.some((e) => !e.removed && e.args.value === cmd.transferAmountWei);
        }
        else {
            return false;
        }
    }, [checkExistingUpfrontTransfer, isSuccessForTransferEvents]);
    const didAllQueriesFinish = useMemo(() => !isLoadingForGetFlowRate &&
    ...
    useEffect(() => didAllQueriesFinish
        ? onMapped?.({ commandId: cmd.id, contractWrites })
        : void 0, [cmd.id, contractWrites, didAllQueriesFinish]);
    return null;
}
const createContractWrite = (arg) => ({
    id: nanoid(),
