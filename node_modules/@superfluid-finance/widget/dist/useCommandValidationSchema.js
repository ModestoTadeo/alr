...
import { cfAv1ForwarderABI, cfAv1ForwarderAddress, hostABI, hostAddress, superfluidGovernanceABI, superTokenABI, } from "./core/index.js";
...
export const useCommandValidationSchema = () => useMemo(() => z
    .object({
    wrapIntoSuperTokensCommand: z
        ...
        message: "You don’t have enough underlying token balance to wrap.",
    })
        .optional(),
    subscribeCommand: z
        .custom()
        .refine((x) => x.accountAddress.toLowerCase() !==
        x.receiverAddress.toLowerCase(), {
        message: "You can't stream to yourself.",
    }),
})
    .refine(async ({ subscribeCommand: cmd }) => {
    const [[_lastUpdated, existingFlowRate, _existingDeposit, _owedDeposit],] = await readContracts({
        allowFailure: false,
        contracts: [
            {
                chainId: cmd.chainId,
    ...
    .refine(async ({ subscribeCommand: cmd, wrapIntoSuperTokensCommand }) => {
    const metadata = superfluidMetadata.getNetworkByChainId(cmd.chainId); // TODO(KK): optimize
    ...
    const [[_lastUpdated, existingFlowRate, existingDeposit, _owedDeposit], minimumDeposit, [liquidationPeriod, _patricianPeriod],] = await readContracts({
        allowFailure: false,
        contracts: [
            {
                chainId: cmd.chainId,
    ...
    const adjustedAvailableBalance = availableBalance -
    ...
    message: "You don’t have enough Super Token balance to cover buffer.",
})
    .refine(async ({ subscribeCommand: cmd, wrapIntoSuperTokensCommand }) => {
    const [accountFlowRate, [availableBalance, _deposit, _owedDeposit, timestamp], [_lastUpdated, existingFlowRate, existingDeposit, _owedDeposit2],] = await readContracts({
        allowFailure: false,
        contracts: [
            {
                chainId: cmd.chainId,
    ...
    const adjustedAvailableBalance = availableBalance -
        ...
        (wrapIntoSuperTokensCommand?.amountWeiFromSuperTokenDecimals ??
            0n);
    const accountFlowRateWithNewFlowRate = accountFlowRate - newFlowRateWei + existingFlowRate;
    ...
    const secondsToCritical = Math.round((criticalDate.getTime() - Date.now()) / 1000);
    const SECONDS_IN_DAY = 86400n;
    return secondsToCritical > SECONDS_IN_DAY;
}, {
    message: "You need to have Super Token balance for at least 24 hours of streaming.",
}), []);
