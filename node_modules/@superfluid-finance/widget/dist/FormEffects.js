...
export function FormEffects() {
    const { watch, resetField, setValue, getFieldState,
    ...
    useEffect(() => void resetField("paymentOptionWithTokenInfo", {
        keepDirty: true,
        keepTouched: true,
        keepError: false,
    }), [network]);
    ...
    useEffect(() => {
        if (paymentOptionWithTokenInfo) {
            ...
            const isPureSuperToken = superToken.extensions.superTokenInfo.type === "Pure";
            if (!isWrapDirty && !isPureSuperToken) {
                const defaultWrapAmountWei = mapFlowRateToDefaultWrapAmount(paymentDetails.defaultWrapAmount, flowRate) +
                    parseEther(paymentOptionWithTokenInfo.paymentOption.transferAmountEther ?? "0");
                const defaultWrapAmountEther = formatEther(defaultWrapAmountWei);
                resetField("wrapAmountInUnits", {
                    keepDirty: false,
                    keepTouched: true,
    ...
    useEffect(() => {
        eventHandlers.onPaymentOptionUpdate(paymentOptionWithTokenInfo?.paymentOption);
